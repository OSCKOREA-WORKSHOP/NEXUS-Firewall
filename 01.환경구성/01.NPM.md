/\*<!\[CDATA\[\*/ div.rbtoc1661719193686 {padding: 0px;} div.rbtoc1661719193686 ul {list-style: disc;margin-left: 0px;} div.rbtoc1661719193686 li {margin-left: 0px;padding-left: 0px;} /\*\]\]>\*/
...
1.  샘플코드 작성 (app.js)
    ...
    실행 (node app.js), 브라우저를 통해서 접속 확인가능
    
    ![](attachments/650019166/653918251.png)
...
![](attachments/650019166/653983800.png)

Nexus Repository 에서 라이브러리 및 의존성 추가 확인

![](attachments/650019166/653918314.png?width=680)
...
1.  Nexus RM에 npm 저장소 추가
    
    *   npm-proxy 추가
        
        ![](attachments/650019166/652050494.png?width=578)
        
        URL이 설정 됩니다.  
        IQ Server 관련된 체크 박스를 설정합니다.  
        Proxy 에 npm Origin 주소를 입력합니다. ( [https://registry.npmjs.org](https://registry.npmjs.org) )
        
    *   npm hosted 추가
        
        ![](attachments/650019166/652050501.png?width=578)
        
    *   npm public 추가 (npm hosted+ npm proxy group)
        
        ![](attachments/650019166/652050507.png?width=578)
        
        Member repositories 에 연관된 저장소를 “>” 버튼을 눌러 추가해 줍니다.  
        순서는 npm hosted, npm-proxy 순으로 설정합니다.  
        라이브러리 검색 시 우선 검색 순입니다.
        
    *   npm 저장소 리스트
        
        ![](attachments/650019166/652050513.png?width=578)
        

* * *

Publish configuration 을 통해서 업로드를 사설 Private Repository 에 가능
...
1.  pakage.json 정보 추가
    ...
    ```js
    {
      ...
      "publishConfig": {
        "registry": "http://nexus-workshop.openmsa.cloud:8081/repository/npm-public/"
      },
      "author": "",
      "license": "ISC",
      "dependencies": {  }
    ...
    pakage.json 파일에 아래 설정 추가 (publishConfig : 업로드시에 사용하는 주소)
